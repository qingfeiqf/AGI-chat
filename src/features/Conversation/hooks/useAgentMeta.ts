...
const LOBE_AI_TITLE = 'AGI-chat AI';
 ...
 * by showing AGI-chat AI title instead of the agent's own meta.
...
export const useAgentMeta = (messageAgentId?: string | null): MetaData => {
  ...
  return useMemo(() => {
    ...
    if (isBuiltinAgent) {
      // Use DB-stored title if customized (e.g. via onboarding), otherwise fallback to AGI-chat AI
